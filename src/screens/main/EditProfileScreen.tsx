    ...
    <SafeAreaView style={GlobalStyles.safeArea}>
      ...
      >
        ...
        {SALARY_OPTIONS.map(([key, label]) => (
          <TouchableOpacity
            key={key}
            style={[styles.salRow, salaryRange === key && styles.salRowOn]}
        ...
        <View style={styles.toggleRow}>
          <TouchableOpacity
            style={[styles.toggle, showOrientation && styles.toggleOn]}
            onPress={() => setShowOrientation(v => !v)}
          >
        ...
        {LOOKING_FOR_OPTIONS.map(([key, label]) => (
          <TouchableOpacity
            key={key}
            style={[styles.salRow, lookingFor === key && styles.salRowOn]}
        ...
        <View style={styles.salarySteps}>
          {[0,40,60,80,100,150,200].map(val => (
            <TouchableOpacity
              key={val}
              style={[styles.stepBtn, minPartnerSalary === val && styles.stepBtnOn]}
              onPress={() => setMinPartnerSalary(val)}
            >
        ...
        <ChipGroup
          options={PROMPT_QUESTIONS}
          selected={[promptQuestion]}
          onChange={(v) => setPromptQuestion(v[0])}
          single columns={1}
        />
        <TextInput
        ...
        <TouchableOpacity
          style={[GlobalStyles.primaryButton, { marginTop: Spacing.xl, marginBottom: 40 }]}
          onPress={handleSave}
          disabled={saving}
        >
          {saving
...
EOFcat > src/screens/main/EditProfileScreen.tsx << 'EOF'
import React, { useState } from 'react'
import {
  View, Text, TextInput, TouchableOpacity, StyleSheet,
  ScrollView, Alert, ActivityIndicator,
